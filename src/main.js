...
document.addEventListener('DOMContentLoaded', () => {
      ...
      chapters: [
        {
          ...
          bgImage: 'https://images.unsplash.com/photo-1576086213369-97a306d36557?auto=format&fit=crop&q=80&w=1000',
          detail: `
            <div class="history-detail">
              <section>
                <h4>🔬 초기 생명 과학의 발견</h4>
                <ul>
                  <li><strong>로버트 훅 (17세기):</strong> 세포 처음 관찰</li>
                  <li><strong>파스퇴르 (19세기):</strong> 생물속생설 입증</li>
                </ul>
              </section>
              <section>
                <h4>🧬 유전학 및 미생물학의 발전</h4>
                <ul>
                  <li><strong>멘델:</strong> 유전 법칙 확립</li>
                  <li><strong>플레밍:</strong> 페니실린 발견</li>
                </ul>
              </section>
              <section>
                <h4>🛰️ 현대 생명 과학과 분자 생물학</h4>
                <ul>
                  <li><strong>왓슨 & 크릭:</strong> DNA 구조 규명</li>
                  <li><strong>게놈 프로젝트:</strong> 유전 정보 해독</li>
                </ul>
              </section>
            </div>
          `
        },
        {
          ...
          detail: `<h3>세포 구조</h3><p>핵, 미토콘드리아 등 소기관 기능을 다룹니다.</p>`
        },
        {
          id: 'bio2-3',
          title: '3. 세포 호흡과 광합성',
          topics: ['효소', '세포 호듭 과정', '광합성 원리'],
          detail: `<h3>에너지 대사</h3><p>TCA 회로 및 광합성 과정을 다룹니다.</p>`
        },
        {
          id: 'bio2-4',
          title: '4. 유전자의 발현과 조절',
          topics: ['DNA 복제', '전사와 번역', '유전자 발현'],
          detail: `<h3>분자 생물학</h3><p>생명 정보의 흐름을 배웁니다.</p>`
        },
        {
          id: 'bio2-5',
          title: '5. 생물의 진화와 다양성',
          topics: ['분류 체계', '진화의 증거', '생명 탄생'],
          detail: `<h3>3역 6계</h3><p>생물 분류와 종 분화 원리를 배웁니다.</p>`
        },
        {
          id: 'bio2-6',
          title: '6. 생명 공학 기술',
          topics: ['유전자 재조합', '줄기 세포', '생명 윤리'],
          detail: `<h3>미래 과학</h3><p>현대 기술과 윤리적 이슈를 다룹니다.</p>`
        }
      ]
    }
  };
  ...
  function renderApp() {
    ...
    bioData.bio1.chapters.forEach(ch => {
      grid1.appendChild(createCard(ch));
      if (currentSubject === 'bio1') tocList.appendChild(createTocItem(ch));
    });

    bioData.bio2.chapters.forEach(ch => {
      grid2.appendChild(createCard(ch));
      if (currentSubject === 'bio2') tocList.appendChild(createTocItem(ch));
    });
  ...
  function createCard(chapter) {
    ...
    card.className = 'card';
    card.innerHTML = `<h3>${chapter.title}</h3><ul>${chapter.topics.map(t => `<li>${t}</li>`).join('')}</ul>`;
  ...
  function openModal(chapter) {
    ...
    modalContent.style.backgroundImage = chapter.bgImage ? `url(${chapter.bgImage})` : 'none';
  ...
  function handleNavClick() {
    if (window.innerWidth <= 1024 && menuToggle && aside) {
